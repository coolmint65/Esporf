...
class TestH2HTrends:
    ...
    def test_h2h_btts_trend(self, db):
        """All 15 H2H matches have both teams scoring (4-3)."""
        _seed_high_scoring_h2h(db)
        analyzer = TrendAnalyzer(db)
        analyzer.min_sample = 10
        analyzer.min_hit_rate = 0.70
        ...
        btts = [t for t in report.trends if t.category == "BTTS - Yes" and t.trend_type == "h2h"]
        assert len(btts) == 1
        assert btts[0].hit_rate == 1.0
